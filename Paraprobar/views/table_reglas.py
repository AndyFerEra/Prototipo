...
from Paraprobar.models.excel_data import Reglas,ExcelData
...
def _header_cell(text: str, icon: str) -> rx.Component:
    return rx.table.column_header_cell(
        rx.hstack(
            ...
            align="center",
            spacing="2",
            min_width="120px",  # Asegura un mínimo de ancho para cada columna
        ),
    )

#muestra los datos en la tabla reglas
def _show_item_reglas(item: Reglas, index: int) -> rx.Component:
    ...
    return rx.table.row(
        rx.table.cell(item.codigo_disciplina),
        rx.table.cell(item.disciplina), 
        rx.table.cell(item.tipo_entregable),
        rx.table.cell(item.codigo_ted),
        rx.table.cell(item.ted),
        rx.table.cell(item.sector),
        rx.table.cell(item.etapa_ingenieria),
...
def file_upload_reglas() -> rx.Component:
    return rx.box(
        rx.heading("Subir Archivo para reglas", size="3", margin_bottom="1rem", color="#2D3748"),
        rx.upload(
            rx.box(
                rx.vstack(
    ...
    return rx.box(
        ...
        rx.flex(
            ...
            rx.button(
                ...
                on_click=rx.redirect("/agregarReglas"),
            ),
            spacing="3",
            justify="between",
            wrap="wrap",
        ...
        rx.table.root(
            rx.table.header(
                ...
                rx.table.row(
                    _header_cell("Cod Disc", "hash"),
                    _header_cell("Disciplina", "list-collapse"),
                    _header_cell("Tipo Entrgbl", "square-stack"),
                    _header_cell("Cod TED", "hash"),
                    _header_cell("TED", "file-spreadsheet"),
                    _header_cell("Sector", "factory"),
                    _header_cell("Etp Ing", "brain"),
                    _header_cell("Estado", "notebook-tabs"),
                ),
            ),
            rx.table.body( 
                rx.foreach(
